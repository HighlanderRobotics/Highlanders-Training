...
- This includes a `class` called `SomethingIO` which defines the real implementation (most of the time) of the hardware interface
...
- `IOInputs` is marked with `@AutoLog` which means all the inputs and outputs for the subsystem will be automatically recorded in the log, so we can play it back later.
    - This generates an `IOInputsAutoLogged` class when the code builds, which can then be sent to the logger using `Logger.processInputs()`.
...
public class PivotIO {
    ...
    public void updateInputs(PivotIOInputs inputs) {
        // Note that the exact calls here are just examples, and don't work if copy pasted.
        // To see an actual implementation, check out module 2.8, or find previous codebases
...
In real code, we would also create a `PivotIOSim` implementation. We will skip that for now, but for more information about that, check the [CTRE docs](https://pro.docs.ctr-electronics.com/en/stable/docs/api-reference/simulation/simulation-intro.html), and see [2.9 Simulation](2.9_Simulation.md).

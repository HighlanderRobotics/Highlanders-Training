...
We'll define these member variables at the top of `DrivetrainIO`.
...
This will need to be set in the `DrivetrainIO` constructor, so that it will get called when a new drivetrain object is created.
...
First, we need to replace all the functionality that moved to `DrivetrainIO` with an instance of `DrivetrainIO`.
Get rid of the `TalonFX` objects and `VoltageOut` requests, since those have all been moved to `DrivetrainIO.`
...
public class DrivetrainSubsystem extends SubsystemBase {
    ...
    public DrivetrainSubsystem(CANbus canbus) {
        this.io = new DrivetrainIO(canbus);
    }
...
Copy over the lines in `DrivetrainSubsystem` about `setControl` over to `setVolts` in `DrivetrainIO`, as well as the `VoltageOut` objects.
...
```
*This still calls the same `setControl` method on the same `TalonFX` objects that it did before*—the only difference is that it now goes through `DrivetrainIO` first.

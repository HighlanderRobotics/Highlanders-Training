...
This method is private to ensure that all direct interaction with hardware happens through a Command (and therefore only has one accessor at a time, enforced by the Command scheduler). Making the method private prevents people from circumventing the "1 command per system at a time" requirement.

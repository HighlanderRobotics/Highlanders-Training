...
The `Module` class, in `Module.java` is one layer of abstraction above `ModuleIO`. Its job is to tell the IO *how* it should move the motors. In addition to some helpers, it has 3 main methods:
...
The drive method is the method that drives the robot (woah really).

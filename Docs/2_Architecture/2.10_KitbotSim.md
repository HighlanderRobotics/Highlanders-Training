...
Change the "visibility" of the two TalonFX objects in `DrivetrainIO` to `protected`. This allows any subclasses (like `DrivetrainIOSim`) to access them, while still preventing other classes from accessing the value (as `public` would).
...
Because this class extends from `DrivetrainIO`, in inherits the `updateInputs()` method and all the hardware interface methods. This avoids code duplication, as we only have to write sim-specific code in `DrivetrainIOSim`.
...
public class DrivetrainSubsystem extends SubsystemBase {
    ...
    public DrivetrainSubsystem(CANbus canbus) {
        this.io = Robot.isReal() ? new DrivetrainIO(canbus) : new DrivetrainIOSim(canbus);
